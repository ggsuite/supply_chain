...
void main() {
  ...
  final allNodes = x.scope.scm.nodes;
  ...
  Future<void> writeScopeTestDot(String result) async {
    await graph.writeGraphToFile(
      result,
      'test/graphs/graph_test.dot',
    );
  }
  ...
  group('Graph2', () {
    group('tree, dot', () {
      group('should throw', () {
        test('when both, node and scope are null', () {
          expect(
            () => graph.tree(node: null, scope: null),
            throwsA(
              isA<ArgumentError>().having(
                (e) => e.message,
                'message',
        ...
        test('when both, node and scope are given', () {
          expect(
            () => graph.tree(node: x, scope: Scope.example()),
            throwsA(
              isA<ArgumentError>().having(
                (e) => e.message,
                'message',
      ...
      group('should print a node', () {
        group('with no suppliers and customers', () {
          group(
            'when only a "node" is given ',
            () {
              test('without highlighting', () async {
                // Create the tree
                final tree = graph.tree(node: x);

                // Create dot
                final dot = graph.dot(tree: tree);
                await writeScopeTestDot(dot);
              ...
              test('with highlighted node', () async {
                // Create the
                final tree = graph.tree(
                  node: x,
                  highlightedNodes: [x],
                );
                ...
                expect(tree.isHighlighted, isFalse);
                expect(tree.nodeItems.length, 1);
                expect(tree.nodeItems.first.isHighlighted, true);

                // Check dot
                expect(dot, contains('style = filled;'));
                expect(dot, contains('fillcolor = "#FFFFAA";'));
              });

              test('with highlighted scope', () async {
                // Create tree
                final tree = graph.tree(
                  node: x,
                  highlightedScopes: [x.scope],
                );
                ...
                // Check tree
                expect(tree.isHighlighted, isTrue);
                expect(tree.nodeItems.length, 1);
                expect(tree.nodeItems.first.isHighlighted, false);
                ...
                await writeScopeTestDot(dot);
                expect(dot, contains('style = filled;'));
                expect(dot, contains('fillcolor = "#AAFFFF88";'));
              });
            },
          );
        });
        ...
        group('with direct suppliers', () {
          test('when supplierDepth == 1', () {
            // Create tree
            final tree = graph.tree(node: x, supplierDepth: 1);

            // Create dot
            final dot = graph.dot(tree: tree);
            writeScopeTestDot(dot);
        ...
        group('with direct customers', () {
          test('when customerDepth == 1', () {
            // Create tree
            final tree = graph.tree(node: x, customerDepth: 1);
        ...
        group('with highlighted', () {
          ...
          test('scopes', () {
            ...
            for (final scope in allScopes) {
              ...
              expect(
                tree.findScopeItem(scope)?.isHighlighted,
                isHighlighted,
              );
            }
